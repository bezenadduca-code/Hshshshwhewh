...
------------------------------------------------------------------------
-- CONFIG SHARE (Copy / Load)
------------------------------------------------------------------------
local sec_cfg = tabInterface:Section({ Title = "Config Share", Opened = true })

-- Copy Config → clipboard
sec_cfg:Button({ Title = "📋 Copy Config", Callback = function()
    pcall(function()
        local flags = win.Flags or {}
        local encoded = svc.Http:JSONEncode(flags)
        if setclipboard then
            setclipboard(encoded)
            -- brief visual feedback via a notification if WindUI supports it
            pcall(function() ui:Notify({ Title = "Config Copied!", Content = "Paste it anywhere to share.", Duration = 3 }) end)
        else
            warn("[hutao] setclipboard not available on this executor")
        end
    end)
end })

-- Load Config from string
local _loadConfigString = ""
sec_cfg:Input({
    Title       = "Load Config String",
    Placeholder = "Paste config JSON here...",
    Flag        = "loadConfigInput",
    Callback    = function(val)
        _loadConfigString = val
    end,
})

sec_cfg:Button({ Title = "⬇ Apply Config", Callback = function()
    pcall(function()
        if _loadConfigString == "" then
            pcall(function() ui:Notify({ Title = "No string!", Content = "Paste a config string first.", Duration = 3 }) end)
            return
        end
        local ok, data = pcall(function() return svc.Http:JSONDecode(_loadConfigString) end)
        if not ok or type(data) ~= "table" then
            pcall(function() ui:Notify({ Title = "Invalid Config", Content = "Could not parse the string.", Duration = 3 }) end)
            return
        end
        -- Push each flag value back into win.Flags and fire the element callback
        for flag, value in pairs(data) do
            pcall(function()
                local el = win:GetElementByFlag(flag)
                if el then
                    if type(el.Set) == "function" then
                        el:Set(value)
                    elseif type(el.SetValue) == "function" then
                        el:SetValue(value)
                    end
                end
        ...
        pcall(function() ui:Notify({ Title = "Config Loaded!", Content = "All settings restored.", Duration = 3 }) end)
    end)
end })
