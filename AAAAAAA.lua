...
local HDT_FLICK_DURATION = combatS.hdtFlickDuration
local HDT_LERP_SPEED     = combatS.hdtFlickSpeed
...
local function combatHDTBeginDrag(killerModel)
    ...
    _hdtFlickConn = svc.Run.RenderStepped:Connect(function(dt)
        ...
        if not _hdtFlickActive then
            local c = _hdtFlickConn
            task.defer(function() if c then c:Disconnect() end end)
            _hdtFlickConn = nil
            return
        end

        local elapsed = tick() - _hdtFlickStartTime
        if elapsed > combatS.hdtFlickDuration then
        ...
        local alpha = math.min(1, dt * combatS.hdtFlickSpeed)
