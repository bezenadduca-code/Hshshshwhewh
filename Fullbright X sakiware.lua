...
local _brightLoop = nil
secFullbright:Toggle({ Title = "Loop Fullbright", Type = "Checkbox", Flag = "loopFullbright", Default = false,
    Callback = function(on)
        if _brightLoop then
            _brightLoop:Disconnect()
            _brightLoop = nil
        end
        if on then
            local Lighting = game:GetService("Lighting")
            ...
            _brightLoop = svc.Run.RenderStepped:Connect(brightFunc)
        end
    end
})
...
task.spawn(function()
    ...
    task.defer(function()
        ...
        if sakiConfig:Get("loopFullbright") then
            local Lighting = game:GetService("Lighting")
            _brightLoop = svc.Run.RenderStepped:Connect(function()
                Lighting.Brightness = 2
                Lighting.ClockTime = 14
                Lighting.FogEnd = 100000
                Lighting.GlobalShadows = false
