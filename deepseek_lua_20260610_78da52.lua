...
local fs = {
    ...
    read      = readfile     or function() return "",
...
• HDT remade to use BodyVelocity — destroys your movement after the Duration slider runs out
...
local combatSetupSoundWatcher = function()
    task.spawn(function()
...
local function combatPlayHDTAnim(hum)
...
local function combatHDTBeginDrag(killerModel, blockTrack)
    pcall(function()
        ...
        local killerRoot =
            killerModel
            and (
                killerModel:FindFirstChild("HumanoidRootPart")
                or killerModel.PrimaryPart
            )
        ...
        if blockTrack then
            blockTrack.Stopped:Connect(stopDrag)
        end

        task.spawn(function()

            local deadline =
                tick() + (combatS.hdtDuration or 0.4)
            ...
            while not stopped and tick() < deadline do
                ...
                local offset = predicted - hrp.Position
...
local function combatOnBlockAnim(track)
    pcall(function()
        ...
        if not BLOCK_ANIMS[id] then
            return
        end

        if not combatS.hdtEnabled then
            return
        end
...
local combatStartLoops = function()
...
local combatStopLoops = function()
...
sec_017:Slider({ Title = "Drag Speed", Flag = "combatHDTMoveSpeed", Value = {Min=1,Max=120,Default=combatS.hdtMoveSpeed}, Step = 1, Callback=function(v) combatS.hdtMoveSpeed=v end
})
