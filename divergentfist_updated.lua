...
-- ──────────────────────────────────────────────
--  MOVE CONTROLLER (ARC DASH → LAND AT BACK)
-- ──────────────────────────────────────────────
local function performCurvedDash(targetRoot)
    ...
    -- TRUE left/right: pick a side offset perpendicular to the line between you and target
    ...
    -- Slower: divide by a much lower effective speed and add a guaranteed minimum
    local totalDuration = math.max((baselineDist / (CONFIG.DashSpeed * 0.6)) * CONFIG.SpinSpeedMultiplier, 0.3)
    ...
    -- Capture positions at dash start so arc is consistent
    local startPos = myPos
    local toTarget = (targetRoot.Position - startPos)
    -- Right vector perpendicular to the line toward target (flat on Y)
    local flat = Vector3.new(toTarget.X, 0, toTarget.Z).Unit
    local perp = Vector3.new(flat.Z, 0, -flat.X) * spinSign  -- left or right side
    ...
    connection = RunService.Heartbeat:Connect(function(dt)
        ...
        local st = t * t * (3 - 2 * t)  -- smoothstep

        local currentTargetPos = targetRoot.Position
        local backPos = (targetRoot.CFrame * CFrame.new(0, 0, CONFIG.BehindOffset)).Position

        -- Arc bulges out to the left or right side at the midpoint
        -- sin(st * pi) peaks at 0.5 giving a natural arc shape
        local arcBulge = math.sin(st * math.pi) * CONFIG.LoopRadius
        local straightPath = startPos:Lerp(backPos, st)
        local frameTargetPos = straightPath + (perp * arcBulge)
        frameTargetPos = Vector3.new(frameTargetPos.X, hrp.Position.Y, frameTargetPos.Z)
...
-- ── SETTINGS TAB ────────────────────────────────
local SettingsTab = Window:Tab({ Title = "Itadori", Icon = "settings" })
