...
combatS.messageOnBlock = false
combatS.messageOnBlockText = ""
combatS.messageOnPunch = false
combatS.messageOnPunchText = ""
combatS.messageBlockDelay = 0
combatS.messagePunchDelay = 0
...
local _combatLastBlockMsgTime = 0
local _combatLastPunchMsgTime = 0
local COMBAT_MSG_BLOCK_CD     = 0.6
local COMBAT_MSG_PUNCH_CD     = 0.6

local function combatSendChat(text)
...
local function combatTryBlockFromSound(sound, preId)
    ...
    local nowMsg = tick()
    if combatS.messageOnBlock and combatS.messageOnBlockText ~= ""
        and (nowMsg - _combatLastBlockMsgTime) > COMBAT_MSG_BLOCK_CD then
        _combatLastBlockMsgTime = nowMsg
        task.spawn(function()
            if combatS.messageBlockDelay > 0 then task.wait(combatS.messageBlockDelay) end
            combatSendChat(combatS.messageOnBlockText)
        end)
    end
end
    ...
    pcall(function()
        ...
        local function restoreMovement()
            ...
            if hum and hum.Parent then
                ...
                -- DO NOT call MoveTo here — it freezes the player until they sprint
                -- instead just re-enable controls and reset speed, that's enough
            end
    ...
    pcall(function()
        ...
        if combatS.autoPunchOn then
            task.delay(0.12, function()
                ...
                local nowMsg = tick()
                if combatS.messageOnPunch and combatS.messageOnPunchText ~= ""
                    and (nowMsg - _combatLastPunchMsgTime) > COMBAT_MSG_PUNCH_CD then
                    _combatLastPunchMsgTime = nowMsg
                    task.spawn(function()
                        if combatS.messagePunchDelay > 0 then task.wait(combatS.messagePunchDelay) end
                        combatSendChat(combatS.messageOnPunchText)
                    end)
                end
            end)
        end
...
sec_015:Toggle({ Title = "Message When Blocking", Flag = "combatMsgOnBlock", Default = combatS.messageOnBlock,
    Callback = function(on) combatS.messageOnBlock = on end, Type = "Checkbox" })

sec_015:Input({ Title = "Block Message", Flag = "combatMsgOnBlockText", Placeholder = "type your message...",
    Default = combatS.messageOnBlockText,
    Callback = function(v) combatS.messageOnBlockText = v end })

sec_015:Slider({ Title = "Block Message Delay (s)", Flag = "combatMsgBlockDelay",
    Value = { Min = 0, Max = 10, Default = combatS.messageBlockDelay }, Step = 0.1,
    Callback = function(v) combatS.messageBlockDelay = v end })
...
sec_016:Toggle({ Title = "Message When Punching", Flag = "combatMsgOnPunch", Default = combatS.messageOnPunch,
    Callback = function(on) combatS.messageOnPunch = on end, Type = "Checkbox" })

sec_016:Input({ Title = "Punch Message", Flag = "combatMsgOnPunchText", Placeholder = "I'm not giving up till every one of them is dead .... or until I am",
    Default = combatS.messageOnPunchText,
    Callback = function(v) combatS.messageOnPunchText = v end })

sec_016:Slider({ Title = "Punch Message Delay (s)", Flag = "combatMsgPunchDelay",
    Value = { Min = 0, Max = 10, Default = combatS.messagePunchDelay }, Step = 0.1,
    Callback = function(v) combatS.messagePunchDelay = v end })
